...
void compileBlock(void)
{
  ...
  compileDeclarations();
  compileBlock2();
  assert("Parsed a Block ....");
}

void compileDeclarations(void)
{
  assert("Parsing a Declarations ....");
  if (lookAhead->tokenType == KW_CONST)
  {
    compileConstDecls();
    compileDeclarations();
...
{
  assert("Parsing constants...");
  eat(KW_CONST);
  compileConstDecl();
  compileConstDecls2();
  assert("Constants parsed!");
}
...
{
  //assert("Parsing subconstants2...");
  while (lookAhead->tokenType == TK_IDENT)
    compileConstDecl();
  //assert("Subconstants parsed2!");
}
...
{
  //assert("Parsing subtypes2...");
  while (lookAhead->tokenType == TK_IDENT)
    compileTypeDecl();
  //assert("Subtypes parsed!2");
}
...
{
  //assert("Parsing variables...2");
  while (lookAhead->tokenType == TK_IDENT)
    compileVarDecl();
  //assert("Variables parsed2");
}
